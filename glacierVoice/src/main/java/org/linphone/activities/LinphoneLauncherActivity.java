...
public class LinphoneLauncherActivity extends Activity implements ServiceWaitThreadListener {
    ...
    public void onServiceReady() {
        ...
        if (useFirstLoginActivity && LinphonePreferences.instance().isFirstLaunch()) {
            classToStart = CognitoLoginAssistantActivity.class;
        } else {

            ProxyConfig proxy = LinphoneManager.getCore().getDefaultProxyConfig();
            if (proxy != null) {
                // load contacts on restarting of app
                importVCardFiles();
    ...
    private void importVCardFiles() {
        ImportVCardFilesTask importVCardFilesTask =
                new ImportVCardFilesTask(getApplicationContext());
